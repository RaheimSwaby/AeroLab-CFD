...
When **Particles** or **Both** is selected, use **Pause/Play** to stop or resume particle motion and choose **0.5×**, **1×**, or **2×** speed. Choose **Small**, **Standard**, or **Large** for point size and **Soft**, **Medium**, or **Strong** for opacity; appearance changes apply immediately without restarting particle travel. Choose **Low**, **Medium**, or **High** density to control visual sampling along the same solved paths. Low is recommended for older or lower-powered devices; High is intended for capable devices and may be slower on older hardware. Density changes safely rebuild the displayed particles but do not change the solver result or the 5,200-particle safety cap. Pause, speed, size, opacity, and density are remembered together in this browser, and resuming retains the selected rate. On a first visit with the operating system's reduced-motion preference enabled, particles start paused; an explicit Play/Pause choice is remembered afterward. These controls are disabled in **Lines** mode or whenever solved particles are unavailable.
