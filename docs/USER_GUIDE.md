...
When **Particles** or **Both** is selected, use **Pause/Play** to stop or resume particle motion and choose **0.5×**, **1×**, or **2×** speed. Choose **Small**, **Standard**, or **Large** for point size and **Soft**, **Medium**, or **Strong** for opacity; appearance changes apply immediately without restarting particle travel. Pause, speed, size, and opacity are remembered together in this browser, and resuming retains the selected rate. On a first visit with the operating system's reduced-motion preference enabled, particles start paused; an explicit Play/Pause choice is remembered afterward. These controls are disabled in **Lines** mode or whenever solved particles are unavailable.
